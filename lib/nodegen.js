...
//CodeGen = require("./lib/../swagger-js-codegen-formdata/bin/swagger2js").CodeGen;
...
function runNpmPack(data) {
  ...
  try {
    child_process.execFileSync(npmCommand, ["pack", "./" + data.module], {
      cwd: data.dst
    });
  } catch (error) {
    console.error(error);
...
function extractKeywords(keywordsStr) {
  ...
  keywords = keywordsStr
    ? keywords.concat(csv.parse(keywordsStr)[0])
    : keywords;
...
function function2node(data, options) {
  ...
  return when.promise(function(resolve, reject) {
    ...
    if (data.name === "function") {
      ...
      // Create package.json
      var packageTemplate = fs.readFileSync(
        path.join(__dirname, "../templates/function/package.json.mustache"),
        "utf-8"
      );
      var packageSourceCode = mustache.render(packageTemplate, params);
      fs.writeFileSync(
        path.join(data.dst, data.module, "package.json"),
        packageSourceCode
      );

      // Create node.js
      var nodeTemplate = fs.readFileSync(
        path.join(__dirname, "../templates/function/node.js.mustache"),
        "utf-8"
      );
      var nodeSourceCode = mustache.render(nodeTemplate, params);
      if (options.obfuscate) {
        nodeSourceCode = obfuscator.obfuscate(nodeSourceCode, {
          stringArrayEncoding: "rc4"
        });
      }
      fs.writeFileSync(
        path.join(data.dst, data.module, "node.js"),
        nodeSourceCode
      );

      // Create node.html
      var htmlTemplate = fs.readFileSync(
        path.join(__dirname, "../templates/function/node.html.mustache"),
        "utf-8"
      );
      var htmlSourceCode = mustache.render(htmlTemplate, params);
      fs.writeFileSync(
        path.join(data.dst, data.module, "node.html"),
        htmlSourceCode
      );

      // Create node_spec.js
      var nodeSpecTemplate = fs.readFileSync(
        path.join(
          __dirname,
      ...
      fs.writeFileSync(
        path.join(data.dst, data.module, "test/node_spec.js"),
        nodeSpecSourceCode
      );

      // Create README.md
      var readmeTemplate = fs.readFileSync(
        path.join(__dirname, "../templates/function/README.md.mustache"),
        "utf-8"
      );
      var readmeSourceCode = mustache.render(readmeTemplate, params);
      fs.writeFileSync(
        path.join(data.dst, data.module, "README.md"),
        readmeSourceCode
      );

      // Create LICENSE file
      var licenseTemplate = fs.readFileSync(
        path.join(__dirname, "../templates/function/LICENSE.mustache"),
        "utf-8"
      );
      var licenseSourceCode = mustache.render(licenseTemplate, params);
      fs.writeFileSync(
        path.join(data.dst, data.module, "LICENSE"),
        licenseSourceCode
      );
  ...
  return when.promise(function(resolve, reject) {
    ...
    var swagger = JSON.parse(JSON.stringify(data.src), function(key, value) {
      if (key === "consumes" || key === "produces") {
        // Filter type of 'Content-Type' and 'Accept' in request header
        ...
        // Handle swagger-js-codegen bug
        return undefined;
      } else if (key === "operationId") {
        // Sanitize 'operationId' (remove special chars)
    ...
    var className = swagger.info.title
      .toLowerCase()
      .replace(/(^|[^a-z0-9]+)[a-z0-9]/g, function(str) {
    ...
    // extract global tags -- CORY
    data.tags = swagger.tags.map(t => t.name);
    ...
    if (options.obfuscate) {
      nodejsSourceCode = obfuscator.obfuscate(nodejsSourceCode, {
        stringArrayEncoding: "rc4"
      });
    }
    fs.writeFileSync(
      path.join(data.dst, data.module, "lib.js"),
      nodejsSourceCode
    );
    ...
    var packageSourceCode = CodeGen.getCustomCode({
      ...
      template: {
        class: fs.readFileSync(
          path.join(__dirname, "../templates/swagger/package.json.mustache"),
          "utf-8"
        ),
        method: "",
        type: ""
      ...
      mustache: {
        ...
        projectAuthor:
          swagger.info.contact && swagger.info.contact.name
    ...
    //     return render("{{camelCaseName}}") !== "body" ||
    //       render("{{methodName}}") !== render("{{camelCaseName}}")
    //       ? render(content)
    //       : "";
    //   };
    // };
    ...
    //   return function(content, render) {
    //     return render("{{camelCaseName}}") === "body" ||
    //       render("{{camelCaseName}}") === render("{{methodName}}")
    //       ? render(content)
    //       : "";
    //   };
    // };
    ...
    var isNotBodyParam = function() {
      return function(content, render) {
        return render("{{camelCaseName}}") !== render("{{methodName}}")
          ? render(content)
          : "";
      };
    };
    ...
    var hasOptionalParams = function() {
      return function(content, render) {
        var params = render(
          "{{#parameters}}{{^required}}{{camelCaseName}},{{/required}}{{/parameters}}"
        );
        return params
          .split(",")
          .filter(p => p)
    ...
    var isGroupMethod = function() {
      return function(content, render) {
        return render("{{methodGroup}}") === "{{selectedGroup}}"
          ? render(content)
          : "";
      };
    };
    ...
    var nodeSourceCode = CodeGen.getCustomCode({
      ...
      template: {
        class: fs.readFileSync(
          path.join(__dirname, "../templates/swagger/node.js.mustache"),
          "utf-8"
        ),
        method: "",
        type: ""
    ...
    var htmlSourceCode = CodeGen.getCustomCode({
      ...
      template: {
        class: fs.readFileSync(
          path.join(__dirname, "../templates/swagger/node.html.mustache"),
          "utf-8"
        ),
        method: "",
        type: ""
    ...
    var languages = fs.readdirSync(
      path.join(__dirname, "../templates/swagger/locales")
    );
    languages.forEach(function(language) {
      ...
      fs.writeFileSync(
        path.join(data.dst, data.module, "locales", language, "node.json"),
    ...
    var readmeSourceCode = CodeGen.getCustomCode({
      ...
      template: {
        class: fs.readFileSync(
          path.join(__dirname, "../templates/swagger/README.md.mustache"),
          "utf-8"
        ),
        method: "",
        type: ""
    ...
    var licenseSourceCode = CodeGen.getCustomCode({
      ...
      template: {
        class: fs.readFileSync(
          path.join(__dirname, "../templates/swagger/LICENSE.mustache"),
          "utf-8"
        ),
        method: "",
        type: ""
